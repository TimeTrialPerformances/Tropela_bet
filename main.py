...
def update_indicator(valor_seleccionado):
    if valor_seleccionado == 'tropela':
        ...
        balances = np.insert(df_bet.groupby('Carrera', as_index=False).first()['total'], 0, 375, axis=0)
        ...
        Rentabilidad = np.insert(df_bet.groupby('Carrera', as_index=False).first()['Rentabilidad'],0, 0, axis= 0)
